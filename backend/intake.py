...
def start_job(uid, name, aoi):
    ...
    threading.Thread(target=_run, args=(job_id, d, info, name, aoi), daemon=True).start()
    return job_id


def _run(job_id, d, info, name, aoi):
    try:
        files = info["files"]
        _set(job_id, stage=0)
        ...
        meta = survey.create(name or "Untitled survey", source, loaded, probs, extracted,
                             {"name": ckpt.get("arch", "U-Net") + " trained on ISPRS Potsdam (RGB + nDSM, height-optional)"})
