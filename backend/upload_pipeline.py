...
def detect_buildings_unsupervised(img_arr, meters_per_px):
    ...
    return contours_to_polys(mask, meters_per_px)


def contours_to_polys(mask, meters_per_px):
    """Blob mask -> simplified pixel polygons, size-filtered in real metres so the
    thresholds hold whatever resolution the upload happens to be."""
...
def unet_checkpoint():
    p = UPLOADS_DIR.parent / "models" / "inria_unet.pt"
    if not p.exists():
        return None
    try:
...
def detect_buildings_unet(img_arr, meters_per_px):
    """The Inria-trained U-Net. Unlike the Otsu fallback this carries real
    supervision to a brand-new area -- it learned footprints from five cities,
    so it needs no ground truth for wherever this upload was taken."""
    from ml import infer_buildings

    mask = infer_buildings.predict_mask(img_arr, meters_per_px)
    mask = cv2.morphologyEx(mask * 255, cv2.MORPH_OPEN, np.ones((3, 3), np.uint8))
...
def process_upload(file_path, center_lat, center_lon, width_m, session_id):
    ...
    if unet_checkpoint():
        detector = "unet_inria"
        polys_px = detect_buildings_unet(arr, meters_per_px)
    else:
        detector = "otsu_unsupervised"
        polys_px = detect_buildings_unsupervised(arr, meters_per_px)
    print(f"[upload] detector={detector}, {len(polys_px)} candidate footprints")
    detected_polys = []
    ...
    unet_note = (
        "Detected with a U-Net trained on the Inria Aerial Image Labeling dataset (real "
        "hand-digitised footprints from five cities), so the detector carries genuine "
        "supervision to this new area. Its published accuracy is measured on Inria's own "
        "held-out cities, not on this upload -- nothing here has ground truth to score "
        "against. Verify detections in the field before acting on them.")
    otsu_note = (
        "Detected with an Otsu brightness/texture threshold, not a trained model -- "
    ...
    (session_dir / "metrics.json").write_text(json.dumps({
        "method": detector,
        "note": unet_note if detector == "unet_inria" else otsu_note,
