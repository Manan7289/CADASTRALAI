...
offset by 1-2 m from the drone image. Labels therefore use three values:
  1   building      -- inside the footprint, eroded by INNER_M
  0   not building  -- outside the footprint, dilated by OUTER_M
  255 ignore        -- the uncertain band around every footprint edge, and nodata
...
def labels_for(geoms, transform, shape_hw, valid):
    inside = rasterize([(g, 1) for g in geoms], out_shape=shape_hw, transform=transform).astype(bool)
...
def main(mosaic, footprints, out_dir):
    out = Path(out_dir)
    ...
    while y - size_m > bottom:
        ...
        while x + size_m < right:
            ...
            lab = labels_for(local, t, (TILE, TILE), valid)
    ...
    meta = {"gsd_m": GSD, "tile_px": TILE, "crs": crs.to_string(), "train_tiles": kept,
            ...
            "label_values": {"0": "not building", "1": "building", "255": "ignore"},
