"""Stacking pilot on Gandhinagar (Kaggle T4). Does feeding the U-Net's output into
an instance-segmentation CNN beat each model on its own?
...
S1_EPOCHS, S2_EPOCHS = int(os.environ.get("S1_EPOCHS", 10)), int(os.environ.get("S2_EPOCHS", 12))
CROP = 512
...
def train_maskrcnn(files, s1_dir, stacked, epochs, steps=None):
    ...
    per_epoch = max(1, (len(files) * 2) // 4)
    total = steps or per_epoch * epochs
...
# ------------------------------------------------------------------ main
def main():
    src = glob.glob("/kaggle/input/**/test/*_mask.png", recursive=True)
    whu = glob.glob("/kaggle/input/**/unet_whu_instance.pt", recursive=True)
    log("gandhinagar:", bool(src), "| whu model:", whu)
    if not src or not whu:
        raise SystemExit("missing inputs")
    gn_root = str(Path(src[0]).parent.parent)
    gn = TMP / "gn"
    subprocess.run([sys.executable, str(WORK / "prepare_gandhinagar.py"), gn_root, str(gn)], check=True)
    train_files = sorted((gn / "train").glob("*.npz")); test_files = sorted((gn / "test").glob("*.npz"))
    log(f"train {len(train_files)} | test {len(test_files)}")
    ...
    for stacked in (False, True):
        mm = train_maskrcnn(train_files[:2], pre, stacked, 1, steps=2)
    ...
    half = len(train_files) // 2
    folds = [train_files[:half], train_files[half:]]
    s1_models = []
    for k in (0, 1):
        ...
        subprocess.run([sys.executable, str(WORK / "train.py"), "--tiles", str(d), "--val-tiles", str(gn / "test"),
                        "--val-max", "10", "--init", whu[0], "--epochs", str(S1_EPOCHS), "--batch", "8",
                        "--crops", "2", "--gsd", "0.30", "--out", str(out)], check=True)
    ...
    for name, stacked in (("B  Mask R-CNN, image only", False), ("D  Mask R-CNN, image + U-Net maps (stacked)", True)):
        m = train_maskrcnn(train_files, s1, stacked, S2_EPOCHS)
        rows = []
        for f in test_files:
    ...
    (WORK / "pilot_results.json").write_text(json.dumps(results, indent=2))
    ...
    fig.tight_layout(); fig.savefig(WORK / "pilot_samples.jpg", dpi=60)
    log("done")
