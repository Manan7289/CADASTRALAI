...
EPOCHS, TIME_CAP_MIN = int(os.environ.get("EPOCHS", 30)), 70
...
def main():
    ...
    roofs = glob.glob("/kaggle/input/**/cadastraai-roofs-v1/**/maskrcnn_stacked.pt", recursive=True) or \
            glob.glob("/kaggle/input/**/maskrcnn_stacked.pt", recursive=True)
    log("roofs v1:", roofs)
    if len(tr) < 1000 or len(va) < 100 or not roofs:
    ...
    xb, yb = next(iter(dl)); model(xb[:2].to(DEV)); score(model, va[:1]); fetch(28.5821, 77.0590)
    log("preflight OK")
    w = torch.tensor([0.0, 1.5, 1.0, 1.0, 1.2, 1.0, 1.3, 1.0, 1.0], device=DEV)
    ...
    model.eval()
    ck = {"state_dict": model.state_dict(), "classes": NAMES, "arch": "unet_resnet50", "in_channels": 3,
          "mean": MEAN.tolist(), "std": STD.tolist(), "gsd_m": 0.3, "history": hist}
    torch.save(ck, WORK / "landcover_oem.pt")
    res = score(model, va)
    res["history"] = hist
    (WORK / "landcover_metrics.json").write_text(json.dumps(res, indent=2))
...
def cities(lc_model):
    sys.path.insert(0, str(Path(glob.glob("/kaggle/input/**/cadastraai-roofs-v1/**/compare_fair.py", recursive=True)[0]).parent))
    import compare_fair as cf
    from shapely.geometry import Polygon
    root = Path(glob.glob("/kaggle/input/**/cadastraai-roofs-v1/**/maskrcnn_stacked.pt", recursive=True)[0]).parent
    nets = [cf.load_unet(root / f"s1_fold{k}.pt") for k in (0, 1)]
    m = cf.build_maskrcnn(6); m.load_state_dict(torch.load(root / "maskrcnn_stacked.pt", map_location="cpu")); m.eval()
    tmp = Path("/kaggle/tmp/lc"); tmp.mkdir(parents=True, exist_ok=True)
    ...
    for r, (name, lat, lon) in enumerate(CITIES):
        ...
        np.save(tmp / (stem + ".npy"), cf.maps_u8(ps, pe, pi))
        np.savez(tmp / (stem + ".npz"), rgb=rgb, inst=np.zeros(rgb.shape[:2], np.int16))
        lab, _ = cf.maskrcnn_labels(m, tmp / (stem + ".npz"), tmp, True)
        ...
        for v in np.unique(lab):
            ...
            if c and cv2.contourArea(max(c, key=cv2.contourArea)) >= 30:
                p = Polygon(max(c, key=cv2.contourArea).reshape(-1, 2)).buffer(0).simplify(1.0)
                if not p.is_empty and p.geom_type == "Polygon":
                    rect = p.minimum_rotated_rectangle
                    q = rect if p.area / max(rect.area, 1e-6) > 0.85 else p
                    cv2.polylines(vis, [np.array(q.exterior.coords).astype(np.int32)], True, (255, 255, 255), 2, cv2.LINE_AA)
                    n_roofs += 1
        share = {NAMES[k]: round(float((lc == k).mean() * 100), 1) for k in range(1, 9)}
        summary[name] = {"roofs_outlined": n_roofs, "land_cover_percent": share}
        Image.fromarray(np.hstack([rgb, np.full((1024, 8, 3), 255, np.uint8), vis])).save(out / f"{stem}.jpg", quality=88)
        ax[r, 0].imshow(rgb); ax[r, 0].set_title(name, fontsize=13)
        ax[r, 1].imshow(vis); ax[r, 1].set_title(f"land cover + {n_roofs} roof outlines", fontsize=13)
